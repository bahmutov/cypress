...
const RUNNABLE_PROPS = 'id title root hookName hookId err state failedFromHookId body speed type duration wallClockStartedAt wallClockDuration timings final _currentRetry _retries'.split(' ')
...
const testAfterRun = function (test, testRef, Cypress) {
  ...
  _runner.hook = function (name, fn) {
    ...
    const changeFnToRunAfterHooks = () => {
      ...
      fn = function (...args) {
        ...
        const testRef = getTestById(test.id)

        testAfterRun(test, testRef, Cypress)
    ...
    switch (name) {
      case 'afterEach': {
        ...
        if (t) {
          ...
          const willRetry = (t.state === 'failed' && t._currentRetry < t._retries)
          ...
          if (this.suite.root && ((t_1 !== _.last(allTests)) && (t_1 !== _.last(tests)) || willRetry)) {
      ...
      case 'afterAll': {
        ...
        if (t) {
          ...
          if (
            (isRootSuite(this.suite) && isLastTest(t_1, allTests)) ||
              (isRootSuite(this.suite.parent) && lastTestThatWillRunInSuite(t_1, siblings)) ||
              (!isLastSuite(this.suite, allTests) && lastTestThatWillRunInSuite(t_1, siblings))
          ) {
            changeFnToRunAfterHooks()
          }
        }
...
const create = function (specWindow, mocha, Cypress, cy) {
  ...
  const onNextError = (runnable, next, err) => {
    ...
    if (err) {
      ...
      if (test._retries === -1) {
        test._retries = getDefaultRetries(Cypress)
      }

      const willRetry = test._currentRetry < test._retries

      if (willRetry) {
        test.state = 'failed'
      }

      if (isBeforeHook && willRetry) {
        test.trueFn = test.fn
        test.fn = function () {
          throw err
        }
